...
export class AstrologyService {
  ...
  private handleError(error: HttpErrorResponse) {
    ...
    if (error.error instanceof ErrorEvent) {
        ...
        if (error.status === 404 && error.error?.message.includes('Dasha or AntarDasha not found')) {
            errorMessage = 'Dasha or AntarDasha not found for this date';
  ...
  getDetailsByCountryAndDateAndCourseAndRaceId(country: string, date: string, course: string, raceNumber: string): Observable<Race[]> {
    ...
    return this.http.get<Race[]>(`${this.baseUrl}/details-Astrology`, { params })
      .pipe(
